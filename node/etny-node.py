...
class EtnyPoXNode:
    ...
    def process_order(self, order_id, metadata=None):
        ...
        if version == 3:
            ...
            payload_hash = metadata[2].split(':')[1]
            input_hash = metadata[3].split(':')[1]
            list_of_ipfs_hashes = [enclave_image_hash, docker_compose_hash, challenge_hash]
            if input_hash is not None and len(input_hash) > 0:
                list_of_ipfs_hashes.append(input_hash)
            ...
            logger.info("Running docker payload-service")
            run_subprocess(
                ['docker-compose', '-f', f'docker/docker-compose-payload-service.yml', 'up', '-d', 'payload-service'],
                logger)

            payload_service_id = run_subprocess(
                ['docker-compose', '-f', 'docker-compose-payload-service.yml', 'ps', '-q'],
                logger
            )

            run_subprocess(
                ['docker', 'cp', f'{payload_service_id}:/payload.py', 'minio-bucket-v3'],
                logger
            )
            ...
            self.wait_for_enclave_v3(bucket_name, 'result.txt', 120)
            run_subprocess([
                'docker-compose', '-f', self.order_docker_compose_file, 'down'
            ], logger)
            ...
            logger.info(f'Result file successfully uploaded to enty-pynity-v3 bucket')
            logger.info('Reading transaction from file')
            status, transaction_data = self.swift_stream_service.get_file_content(bucket_name, "transaction.txt")
            if not status:
                logger.info(transaction_data)
            ...
            result = self.build_result_format_v3(result_hash, transaction_hex)
            logger.info(f'Result is: {result}')
            logger.info('Adding result to order')
            self.add_result_to_order(order_id, result)
    ...
    def wait_for_enclave_v3(self, bucket_name, object_name, timeout=120):
        i = 0
        while True:
            time.sleep(1)
            i = i + 1
